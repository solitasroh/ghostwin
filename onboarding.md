...
| Phase | 목표 | 완료 기준 | 상태 |
|-------|------|----------|:----:|
| **Phase 1** | libghostty-vt Windows 빌드 검증 | DLL 빌드, C 브릿지 + VtCore 래퍼, 7개 테스트 PASS | **완료** |
...
### Phase 1 완료 산출물 (2026-03-29)

| 산출물 | 경로 |
|--------|------|
| Zig 빌드 스크립트 | `scripts/build_libghostty.ps1` |
| CMake 빌드 스크립트 | `scripts/build_ghostwin.ps1` |
| C 브릿지 (순수 C) | `src/vt-core/vt_bridge.h`, `vt_bridge.c` |
| C++ 래퍼 | `src/vt-core/vt_core.h`, `vt_core.cpp` |
| 테스트 (7/7 PASS) | `tests/vt_core_test.cpp` |
| ADR 3건 | `docs/adr/001~003` |
| 트러블슈팅 가이드 (8건) | `docs/00-research/troubleshooting-windows-build.md` |
...
| ~~libghostty-vt 빌드 시 libxml2 이슈~~ | ~~높음~~ | ~~vcpkg 사전 설치로 우회~~ | **해결됨** (PR #11698) |
| GNU static lib ↔ MSVC 링커 불호환 | **높음** | DLL 방식으로 GNU/MSVC 격리 (ADR-003) | 대응 완료 |
| 한국어 Windows + Ninja 로케일 충돌 | **중간** | `/showIncludes` 접두사 영어 패치 (`build_ghostwin.ps1`) | 대응 완료 |
...
### 프로젝트 현황 (2026-03-29 기준)
- Ghostty: v1.3.1 안정, MIT
- libghostty-vt: v0.1.0, API 불안정, Zig 0.15.2+ 필요
- **Phase 1 완료**: DLL 빌드 + C 브릿지 + VtCore 래퍼, 7/7 테스트 PASS
...
| 항목 | 값 |
|------|-----|
| Zig | 0.15.2 (`-Dtarget=x86_64-windows-gnu -Dsimd=false`) |
| VS | 2026 Community (v18), MSVC 14.51 (`-vcvars_ver=14.51`) |
| CMake | 4.0 + Ninja |
| 링킹 | DLL (`ghostty-vt.dll` + import lib) — ADR-003 |
| 테스트 | 7/7 PASS (create, write, ansi, render, resize, lifecycle, dirty_reset) |

---

*GhostWin Terminal — Onboarding Document v0.3*
*최종 업데이트: 2026-03-29*
