...
| **Phase 6** | **AI 에이전트 특화** | OSC hooks, 알림 링, 에이전트 배지, Named pipe 서버 | ✅ **완료** (6-A 93% + 6-B 97% + 6-C 95%) |
...
M-11 세션 복원 ✅
  → M-11.5 E2E 자동화 ✅
  → 🎯 Phase 6-A: OSC hook + 알림 링 ✅ (93%)
  → 🎯 Phase 6-B: 알림 패널 + 배지 + Toast ✅ (97%)
  → 🎯 Phase 6-C: Named pipe + git ✅ (95%)
  → M-12 설정 UI ← 다음
  → M-13 한글 입력 미리보기
  → M-14 렌더 스레드 안전성
```

**Phase 6 완료 (2026-04-17)**:
- 비전 ② AI 에이전트 멀티플렉서 핵심 기능 전체 실증
- OSC 간접 + Named Pipe 직접 이중화 완성
- Windows에서 유일하게 알림 패널 + 미읽음 점프 + 에이전트 배지 + Claude Code Hooks 직접 연결을 제공하는 터미널
