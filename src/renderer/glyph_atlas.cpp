...
GlyphEntry GlyphAtlas::Impl::rasterize_glyph(ID3D11DeviceContext* ctx,
    ...
    //
    // CreateGlyphRunAnalysis rejects DWRITE_RENDERING_MODE_DEFAULT and
    // DWRITE_RENDERING_MODE_OUTLINE — only raster modes are accepted. Passing
    // DEFAULT triggers a first-chance C++ ArgumentException inside DWrite per
    // glyph and forces the fallback path on every cache miss.
    // https://learn.microsoft.com/en-us/windows/win32/api/dwrite/nf-dwrite-idwritefactory-createglyphrunanalysis
    // GetRecommendedRenderingMode resolves the system-recommended raster mode
    // for the (font, emSize, dpiScale) tuple.
    DWRITE_RENDERING_MODE rendering_mode = DWRITE_RENDERING_MODE_NATURAL_SYMMETRIC;
    HRESULT hr_mode = face_to_use->GetRecommendedRenderingMode(
        em_size, dpi_scale, DWRITE_MEASURING_MODE_NATURAL,
        nullptr, &rendering_mode);
    if (FAILED(hr_mode) ||
    ...
    HRESULT hr = dwrite_factory->CreateGlyphRunAnalysis(&glyph_run,
        dpi_scale,
        nullptr,
        rendering_mode,
        ...
        &analysis);
    if (FAILED(hr)) {
        // Safety net: any future per-glyph mode rejection still falls through.
