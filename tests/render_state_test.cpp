...
static bool test_allocate_and_access() {
    ...
    const auto f = state.acquire_frame_copy();
...
static bool test_resize_preserves_content() {
    ...
    const auto f2 = state.acquire_frame_copy();
...
static bool test_resize_shrink_then_grow_preserves_content() {
    ...
    // Sanity pre-check.
    if (state.acquire_frame_copy().row(0)[0].codepoints[0] != 'S') {
...
static bool test_reshape_metadata_only() {
    ...
    const size_t cap_before = state.acquire_frame_copy().cell_buffer.size();
...
static bool test_reshape_capacity_retention() {
    ...
    state.resize(1, 1);
    const auto mid = state.acquire_frame_copy();
    if (mid.cap_cols != 40 || mid.cap_rows != 5) {
        printf("(mid cap %ux%u != 40x5) ",
               mid.cap_cols, mid.cap_rows);
        return false;
    }
...
static bool test_real_app_resize_flow() {
    ...
    if (state.acquire_frame_copy().row(0)[0].codepoints[0] != 'R') {
...
static bool test_cls_clears_cells() {
    ...
    if (state.acquire_frame_copy().row(0)[0].codepoints[0] != 'H') {
...
static bool test_esc_k_erase_line() {
    ...
    if (state.acquire_frame_copy().row(0)[0].codepoints[0] != 'L') {
...
static bool test_scroll_blanks_new_rows() {
    ...
    const auto before_scroll = state.acquire_frame_copy();
    if (before_scroll.row(0)[0].codepoints[0] != 'A' ||
        before_scroll.row(1)[0].codepoints[0] != 'B' ||
        before_scroll.row(2)[0].codepoints[0] != 'C') {
