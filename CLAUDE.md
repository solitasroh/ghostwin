...
- [x] **P0-* first-pane-render-failure** — bisect-mode-termination v0.1 §5 R2 (HostReady race, "High × Low~Medium") 의 **최초 reproduction + root cause confirmation + Option B structural fix**. **Amended 2026-04-09**: 사용자 hardware verification 에서 "분할 시 첫 세션 사라짐" secondary regression 발견 → 원인 = `TerminalRenderState::resize` 가 1D row-major buffer 를 `_api.allocate()` 로 zero-init 하고 ghostty VT 는 resize 만 했을 때 row 를 dirty 로 report 하지 않아 content 가 복구 안 됨. Fix = content-preserving row-by-row memcpy resize (`4492b5d fix(render): preserve cell buffer across resize`). Unit test 7/7 PASS (신규 2 건: `test_resize_preserves_content` shrink + `test_resize_grow_preserves_content` grow). E2E visual verify 는 bash session keyboard injection 제약으로 불가 → follow-up `first-pane-manual-verification` 에서 처리. 17:04 run 의 MQ-2 evaluator PASS 는 false positive 였음 (좌측 pane "어두운 배경" 이 사실은 content-loss). Archive Appendix A 에 full narrative. Phase 1 진단 = RenderDiag instrumentation (172 LOC 신규, KeyDiag mirror) + `repro_first_pane.ps1` 30회 cold-start harness (539 LOC) + 5-pass evidence-first falsification (e2e-ctrl-key-injection §11.6 패턴 재사용). Phase 2 fix = `_initialHost` 필드 폐기 + `PaneContainerControl` single owner + 첫 pane = split pane 동일 코드 경로 + `gw_render_init` `allow_null_hwnd=true` (Q-A4) + `surface_manager.cpp` DXGI cast `LOG_E` (HC-1, bisect R3 logging extension) + `PaneContainerControl.Initialize` 안 `RegisterAll` 직접 호출 (HC-4) + `PaneLayoutService.OnHostReady` silent return `Trace.TraceError` (HC-2). **7 hidden complexity (HC-1~HC-7) 발굴** — design 시점 lock-in. **R10 unplanned discovery**: `TsfBridge.OnFocusTick` dead-code trap 이 Option B 의 parent HWND (non-top-level → top-level) 전환으로 activate, post-fix 사용자 hardware verification 에서 "key/mouse freeze" catch → commit `9467c9f` 로 same-cycle hotfix. **Evaluator result**: MQ-1 PASS (medium confidence), 전체 7/8 — MQ-7 sidebar click 은 **cascade 가설 decisive 기각** (본 cycle evaluator 에서 MQ-1 PASS + MQ-7 여전히 FAIL) 후 independent regression 확정. Match Rate gap analysis: core fix sub-score ~95%, full cycle closeout 82-88% (manual + hardware-dependent gates open). bisect v0.4 §10.3 cross-cycle retroactive entry 로 R2 CLOSED 표시. 6 commit closeout: `9f7a46d` (Plan+Design) / `57f7833` (RenderDiag) / `2d2f47f` (repro harness) / `f2882d7` (hwnd-less engine init + DXGI cast LOG_E) / `f89e299` (Option B core) / `9467c9f` (TsfBridge hotfix). (2026-04-08). **Follow-up cycles triggered**: `e2e-mq7-workspace-click` (HIGH, independent regression), `repro-script-fix` (MEDIUM, AMSI window-capture block 우회), `first-pane-manual-verification` (MEDIUM, G5b IME + G5c MicaBackdrop 사용자 smoke), `adr-011-timer-review` (LOW, focus timer dead code), `render-overhead-measurement` (LOW, G8/G9 RenderDiag off/on 측정)
