...
빌드/행동 규칙은 `.Codex/rules/`에 분리되어 경로별 자동 로드.

| 규칙 파일 | 적용 범위 |
|-----------|-----------|
| `.Codex/rules/behavior.md` | 항상 (의존성 대응, 빌드 실패, 스크립트) |
| `.Codex/rules/commit.md` | 항상 (커밋 메시지 형식, AI 언급 금지) |
| `.Codex/rules/documentation.md` | 항상 (설명/설계/계획/보고 문서 — 쉬운 한국어 + 다이어그램 + 비교표) |
| `.Codex/rules/build-environment.md` | GhostWin.sln, *.vcxproj, *.csproj, scripts/, external/ghostty/ |
