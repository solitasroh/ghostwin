...
### 3.2 실행 순서 (10명 에이전트 합의 + 코드 품질 분석 기반)

| 순서 | 변경 | 파일 | 근거 | 검증 |
|:----:|------|------|------|------|
| **1** | **3-pass 잔재 제거** (bg_copy_tex, bgTexture, bg_count, 모순 주석) | shader_ps.hlsl, dx11_renderer.cpp | 코드 분석: CRITICAL 3건 | 빌드 + 테스트 |
| **2** | **DWrite 감마 보정 복원** (EnhanceContrast + AlphaCorrection) | shader_ps.hlsl | 10명 전원 합의, 수학 증명. 이전 실패는 이중 감마(CreateAlphaTexture gamma=1.8 + 셰이더 감마) | 스크린샷 |
| **3** | **Bounds 통일**: CreateGlyphRunAnalysis 제거, D2D bounds 사용 | glyph_atlas.cpp | Agent 5,10: WT는 D2D 자체 bounds 사용 | 글리프 정렬 확인 |
| **4** | **미사용 코드 정리** (shader_common.hlsl, ShaderInclude, config 멤버) | 다수 | 코드 분석: WARNING 8건 | 빌드 + 테스트 |
| **5** | SetMatrixTransform 추가 (DPI scale 상쇄) | winui_app.cpp | Agent 8: WT 필수 패턴 | 고DPI 대응 |
| **6** | **디버그 잔재 정리** (atlas_dump #ifdef, static 변수) | winui_app.cpp, glyph_atlas.cpp | 코드 분석: Release 빌드 영향 | 빌드 |
