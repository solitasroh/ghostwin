...
> **Revision**: 2.0 (design-validator + code-analyzer 검증 반영)
...
|---------|---------|---------|
| Microsoft.WindowsAppSDK | 1.8.x | WinUI3 런타임 + 헤더 |
| Microsoft.Windows.CppWinRT | 2.0.x | C++/WinRT projection 헤더 생성 |
| Microsoft.Windows.SDK.BuildTools | 10.0.x | `cppwinrt.exe` 도구 (Plan에 없으나 헤더 생성에 필수) |
...
int WINAPI wWinMain(...) {
    // Bootstrap 초기화 — Runtime 미설치 시 사용자 안내
    ...
    // DispatcherQueue 생성 (Unpackaged 필수 — 자동 생성 안 됨)
...
class GhostWinApp : public winrt::Microsoft::UI::Xaml::ApplicationT<GhostWinApp> {
public:
...
private:
    ...
    // Render thread control (acquire/release ordering for resize protocol)
    ...
    bool m_cursor_blink_visible = true;
...
void GhostWinApp::StartTerminal(uint32_t width_px, uint32_t height_px) {
    ...
    // 렌더 스레드 시작 (staging 할당 이후)
    m_render_running.store(true);
    std::thread([this] { RenderLoop(); }).detach();
    // 또는 ThreadPool::RunAsync 사용 (Plan FR-03)
}
```
...
scfg.on_exit = [self = get_strong()](uint32_t code) {
    ...
    // UI 스레드로 마샬링 (PostQuitMessage 대신)
    self->m_window.DispatcherQueue().TryEnqueue([self]() {
        self->m_render_running.store(false);
        self->m_window.Close();
    });
};
```
