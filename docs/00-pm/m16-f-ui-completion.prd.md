...
| 관점 | 한 줄 요약 |
|---|---|
| **Problem** | M-16-A/B/C/D + Hotfix 2026-05-06 closure 후 잔여 UI 결함 24건 발굴 (P1 1 + P2 13 + P3 10). 핵심: **ToolTip 6% 만 명시 / 캡션 버튼 a11y Name 누락 / 워크스페이스 close ✕ Name 누락 / Tab edge case / 한국어 i18n 부재 / 최대화 하단 padding 시각 검증** |
| **Solution** | P1 + P2 핵심 묶음을 단일 사이클 (1.5-2주) 로 마감. 자동화 검증 도구 (xunit + FlaUI) 도입으로 시각 결함 직접 측정 — 사용자 PC 의존 제거 |
| **Function / UX 효과** | 모든 visible button ToolTip 명시 + a11y Name 100% + Tab navigation chrome 순환 정확 + 한국어 UI 옵션 + 최대화 시 잘림 0px |
...
| **A5** | i18n 영어 hardcode 100% — 한국어 UI 부재 | XAML grep |
...
### 2.3 P3 — 누적 정리 가치 (10건, 후속)

L2 / L5 / NEW-C / C-NEW-2 / F11 / F14 / A3 / A4 / A6 / A7 / A8 — M-16-G 또는 후속 사이클.
...
| 3 | 영어 UI 만 사용 가능 | Settings 에서 한국어 옵션 선택 가능 |
| 4 | 최대화 시 터미널 하단 잘림 | 사방 균등 padding 분배, 잘림 0px |
| 5 | Tab 키 일부 영역에서 stuck | Sidebar / Settings / Pane chrome / NotifPanel ring 정확 순환 |
| 6 | NotifPanel 우클릭 시 메뉴 안 뜸 | 4영역 ContextMenu 일관성 (Sidebar/Pane/Terminal/NotifPanel) |
| 7 | Ctrl+Wheel / Shift+Wheel 단축키 부재 | 폰트 줌 + 스크롤백 |

## 4. Scope

### In Scope

- P1 1건 (L6) + P2 15건 = **16 결함** 마감
...
- P3 10건 — M-16-G 후속 사이클로 분리
- A5 i18n 한국어 — Scope 안 (간단 한국어 1차 적용). 17 언어 cmux parity 는 별도 M-16-I 사이클
- A6 FlowDirection RTL — 한국어 사용자 우선이라 후순위
...
| 한국어 hardcode strings | XAML grep | 5건 이상 한국어 옵션 (i18n 완전 지원은 후속) |
...
| **i18n A5** | 한국어 한 차로 도입 시 누락 가능 — 1차 영어 OR 한국어 옵션만 | Settings 의 culture 옵션 + 핵심 string 한국어 전환 |
...
> **Beachhead**: 한국어 사용자 본인 (= 프로젝트 owner) — 한국어 UI 옵션 + a11y 완전 + ToolTip + 최대화 안정 = 일상 사용 marginal pain 청산
