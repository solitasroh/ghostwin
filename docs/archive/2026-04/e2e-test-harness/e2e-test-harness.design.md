...
| C8 | **R4 (Ctrl+키 미전달) empirically confirmed (2026-04-08 Step 10 self-test)**: 4가지 input layer 시도 모두 실패. (a) `pywinauto.Application(uia).type_keys`, (b) `pywinauto.keyboard.send_keys` standalone, (c) ctypes SendInput batch (atomic), (d) +AttachThreadInput +scancode +cross-thread SetFocus. Alt+V/H, mouse click, window resize는 모두 동작하지만 Ctrl+T / Ctrl+Shift+W만 GhostWin OnTerminalKeyDown에 도달 안 함. (d)는 Alt+V도 break시키는 부작용으로 net negative — revert. 현재 채택 = (c). **Root cause 미확정 (확실하지 않음)** — GhostWin source side WPF KeyBinding/PreviewKeyDown 분기 디버깅 필요 | CTO Lead self-test (4 attempts) | Match Rate 5/8 = 62.5% 의 root cause. R4 follow-up은 별개 PDCA cycle |
...
| R4 | pywinauto `type_keys('%v')`가 WPF `OnTerminalKeyDown`에 도달 안 함 (SystemKey 매핑 이슈) | High | **CONFIRMED (2026-04-08)** | **Empirically confirmed**: Step 10 self-test에서 4가지 input layer 모두 시도. (a) pywinauto Application+UIA type_keys, (b) pywinauto.keyboard.send_keys standalone, (c) ctypes SendInput batch, (d) +AttachThreadInput +scancode +cross-thread SetFocus. **Alt+키, mouse, resize는 모두 통과**, **Ctrl+T / Ctrl+Shift+W만 미전달**. (d)는 Alt+V도 break — net negative, revert. 현재 final = (c) ctypes batch. Match Rate 5/8 = 62.5%. Root cause 미확정 (GhostWin WPF KeyBinding/PreviewKeyDown 분기 디버깅 필요). **R4 follow-up은 별개 PDCA cycle** (e2e-ctrl-key-injection 또는 ghostwin-wpf-keyboard-debug). 본 e2e harness는 Operator/Capture/Window/Mouse/Resize layer 모두 검증 완료 |
...
| 0.1.2 | 2026-04-08 | **R4 self-test 결과 반영** (Step 10 finalization): C8 (R4 empirically confirmed via 4 attempts) 추가, R10 likelihood Low → **CONFIRMED**, mitigation 칸에 4 attempts 결과 + final = ctypes batch + Match Rate 5/8 = 62.5% 명시. Ctrl+키 미전달 root cause는 GhostWin WPF KeyBinding/PreviewKeyDown 디버깅이 필요해 본 feature scope 외로 분리. 별개 PDCA cycle (e2e-ctrl-key-injection)에서 follow-up 권장 | 노수장 (CTO Lead) |
